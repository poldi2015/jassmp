...
public class SynchronizedAsyncTask
    ...
    public void execute( final int delayInMillis, final Executor executor ) {
        if( isDisposed() ) throw new IllegalStateException( "Task is disposed" );
    ...
    @Override protected final void onProgressUpdate( Void... values ) {
    ...
    @Override protected void onCancelled( SynchronizedAsyncTask.Executor runner ) {
        runner.cancel();
        release();
    }

    @Override protected final Executor doInBackground(
            final SynchronizedAsyncTask.Executor... runner ) {
    ...
    @Override protected final void onPostExecute( final SynchronizedAsyncTask.Executor result ) {
        // Die if already disposed
        if( !acquire() ) return;
        try {
