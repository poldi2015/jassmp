 ...
 */
package com.jassmp.NowPlayingQueueActivity;
    ...
    public View getView( final int position, View convertView, ViewGroup parent ) {
        ...
        if( convertView == null ) {
            convertView = LayoutInflater.from( mContext )
                                        .inflate( R.layout.playlist_flipped_listview_layout, parent, false );
            ...
            holder.songTitleText.setPaintFlags(
                    holder.songTitleText.getPaintFlags() | Paint.ANTI_ALIAS_FLAG | Paint.SUBPIXEL_TEXT_FLAG );
            ...
            holder.artistText.setPaintFlags(
                    holder.artistText.getPaintFlags() | Paint.ANTI_ALIAS_FLAG | Paint.SUBPIXEL_TEXT_FLAG );
        ...
        mApp.getAudioPlaybackService().getCursor().moveToPosition( mPlaybackIndecesList.get( position ) );
        ...
        String songArtist;
        try {
            songTitle = mApp.getAudioPlaybackService()
                            .getCursor()
                            .getString( mApp.getAudioPlaybackService()
                                            .getCursor()
                                            .getColumnIndex( DBAccessHelper.SONG_TITLE ) );
            songFilePath = mApp.getAudioPlaybackService()
                               .getCursor()
                               .getString( mApp.getAudioPlaybackService()
                                               .getCursor()
                                               .getColumnIndex( DBAccessHelper.SONG_FILE_PATH ) );
            songArtist = mApp.getAudioPlaybackService()
                             .getCursor()
                             .getString( mApp.getAudioPlaybackService()
                                             .getCursor()
                                             .getColumnIndex( DBAccessHelper.SONG_ARTIST ) );
        } catch( Exception e ) {
        ...
        } else if( mCurrentTheme.equals( "LIGHT_CARDS_THEME" ) || mCurrentTheme.equals( "LIGHT_THEME" ) ) {
            convertView.setBackgroundColor( 0xFFFFFFFF );
            holder.songTitleText.setTextColor( UIElementsHelper.getThemeBasedTextColor( mContext ) );
            holder.artistText.setTextColor( UIElementsHelper.getSmallTextColor( mContext ) );
            holder.removeSong.setImageResource( R.drawable.cross );

        } else if( mCurrentTheme.equals( "DARK_CARDS_THEME" ) || mCurrentTheme.equals( "DARK_THEME" ) ) {
