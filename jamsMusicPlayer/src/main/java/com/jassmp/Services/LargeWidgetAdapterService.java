...
class StackRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    public StackRemoteViewsFactory( Context context, Intent intent ) {
        ...
        cursor = mApp.getAudioPlaybackService().getCursor();
        ...
        if( position <= getCount() ) {
            ...
            try {
                if( mApp.getAudioPlaybackService().getPlaybackIndecesList() != null
                    && mApp.getAudioPlaybackService().getPlaybackIndecesList().size() != 0 ) {
                    if( cursor.getCount() > mApp.getAudioPlaybackService().getPlaybackIndecesList().get( position ) ) {
                        cursor.moveToPosition(
                                mApp.getAudioPlaybackService().getPlaybackIndecesList().get( position ) );
                    } else {
                        return null;
                    }
            ...
            try {
                songDurationInMillis = Long.parseLong(
                        cursor.getString( cursor.getColumnIndex( DBAccessHelper.SONG_DURATION ) ) );
            } catch( Exception e ) {
