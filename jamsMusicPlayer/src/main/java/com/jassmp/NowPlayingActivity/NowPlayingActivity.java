 ...
 */
package com.jassmp.NowPlayingActivity;
...
import com.jassmp.Animations.FadeAnimation;
...
public class NowPlayingActivity extends FragmentActivity {
    ...
    private void initViewPager() {
        ...
        try {
            ...
            mViewPager.setCurrentItem( mApp.getAudioPlaybackService().getCurrentSongIndex(), false );
    ...
    private void animateInControlsBar() {
        android.view.animation.TranslateAnimation slideUp = new android.view.animation.TranslateAnimation(
                Animation.RELATIVE_TO_SELF, 0.0f, Animation.RELATIVE_TO_SELF, 0.0f, Animation.RELATIVE_TO_SELF, 2.0f,
     ...
     */
    private void scrollViewPager( int newPosition, boolean smoothScroll, int velocity, boolean dispatchToListener ) {
    ...
    private void setRepeatButtonIcon() {
        if( mApp.isServiceRunning() ) {
            if( mApp.getAudioPlaybackService().getRepeatMode() == Common.REPEAT_OFF ) {
                mRepeatButton.setImageResource( UIElementsHelper.getIcon( mContext, "repeat" ) );
            } else if( mApp.getAudioPlaybackService().getRepeatMode() == Common.REPEAT_PLAYLIST ) {
                mRepeatButton.setImageResource( R.drawable.repeat_highlighted );
            } else if( mApp.getAudioPlaybackService().getRepeatMode() == Common.REPEAT_SONG ) {
                mRepeatButton.setImageResource( R.drawable.repeat_song );
            } else if( mApp.getAudioPlaybackService().getRepeatMode() == Common.A_B_REPEAT ) {
    ...
    private void setShuffleButtonIcon() {
        if( mApp.isServiceRunning() ) {
            if( mApp.getAudioPlaybackService().isShuffleOn() == true ) {
    ...
    private void setSeekbarDuration( int duration ) {
        ...
        mSeekbar.setProgress( mApp.getAudioPlaybackService().getCurrentMediaPlayer().getCurrentPosition() / 1000 );
        mHandler.postDelayed( seekbarUpdateRunnable, 100 );
    }
    ...
    private OnSeekBarChangeListener seekBarChangeListener = new OnSeekBarChangeListener() {
        ...
        public void onProgressChanged( SeekBar seekBar, int seekBarPosition, boolean changedByUser ) {
            ...
            try {
                long currentSongDuration = mApp.getAudioPlaybackService().getCurrentMediaPlayer().getDuration();
        ...
        public void onStopTrackingTouch( SeekBar seekBar ) {
            ...
            mApp.getAudioPlaybackService().getCurrentMediaPlayer().seekTo( seekBarPosition * 1000 );
    ...
    private OnClickListener repeatButtonClickListener = new OnClickListener() {
        ...
        public void onClick( View arg0 ) {
            ...
            mApp.getAudioPlaybackService().clearABRepeatRange();
            if( mApp.getAudioPlaybackService().getRepeatMode() == Common.REPEAT_OFF ) {
                mRepeatButton.setImageResource( R.drawable.repeat_highlighted );
                mApp.getAudioPlaybackService().setRepeatMode( Common.REPEAT_PLAYLIST );

            } else if( mApp.getAudioPlaybackService().getRepeatMode() == Common.REPEAT_PLAYLIST ) {
                mRepeatButton.setImageResource( R.drawable.repeat_song );
                mApp.getAudioPlaybackService().setRepeatMode( Common.REPEAT_SONG );

            } else {
                mRepeatButton.setImageResource( UIElementsHelper.getIcon( mContext, "repeat" ) );
                mApp.getAudioPlaybackService().setRepeatMode( Common.REPEAT_OFF );
    ...
    private OnClickListener shuffleButtonClickListener = new OnClickListener() {
        ...
        public void onClick( View arg0 ) {
            ...
            boolean shuffleOn = mApp.getAudioPlaybackService().toggleShuffleMode();
    ...
    private OnClickListener mOnClickNextListener = new OnClickListener() {
        ...
        public void onClick( View arg0 ) {
            ...
            if( newPosition < mViewPagerAdapter.getCount() ) {
                ...
                if( mApp.getAudioPlaybackService().getRepeatMode() == Common.REPEAT_PLAYLIST ) {
                    mViewPager.setCurrentItem( 0, false );
                } else {
    ...
    /*private void pinSong() {
		...
		if (mApp.isFetchingPinnedSongs()==false) {
			...
			mApp.getAudioPlaybackService().getCursor().moveToPosition(mApp.getAudioPlaybackService().getPlaybackIndecesList().get(mApp.getAudioPlaybackService().getCurrentSongIndex()));
			
			//Get the song's ID/title.
			String songID = mApp.getAudioPlaybackService().getCursor().getString(mApp.getAudioPlaybackService().getCursor().getColumnIndex(DBAccessHelper.SONG_ID));
			String songTitle = mApp.getAudioPlaybackService().getCursor().getString(mApp.getAudioPlaybackService().getCursor().getColumnIndex(DBAccessHelper.SONG_TITLE));
    ...
    private DrawerListener mDrawerListener = new DrawerListener() {
        ...
        public void onDrawerClosed( View drawer ) {
            if( mQueueDrawerFragment != null && drawer == mCurrentQueueLayout ) {
    ...
    private VelocityViewPager.OnPageChangeListener mPageChangeListener = new VelocityViewPager.OnPageChangeListener() {
        ...
        public void onPageScrolled( final int pagerPosition, float swipeVelocity, int offsetFromCurrentPosition ) {
            ...
            if( mApp.isServiceRunning() && mApp.getAudioPlaybackService().getCursor().getCount() != 1 ) {
				 ...
				 */
                if( swipeVelocity == 0.0f && pagerPosition != mApp.getAudioPlaybackService().getCurrentSongIndex() ) {
                    if( USER_SCROLL ) {
                        mHandler.removeCallbacks( seekbarUpdateRunnable );
                        ...
                        mHandler.postDelayed( new Runnable() {
                            ...
                            @Override
                            public void run() {
                                mApp.getAudioPlaybackService().skipToTrack( pagerPosition );
                            }
    ...
    public Runnable seekbarUpdateRunnable = new Runnable() {
        ...
        public void run() {
            ...
            try {
                long currentPosition = mApp.getAudioPlaybackService().getCurrentMediaPlayer().getCurrentPosition();
        ...
        public int getCount() {
            ...
            try {
                if( mApp.getAudioPlaybackService().getPlaybackIndecesList() != null ) {
                    return mApp.getAudioPlaybackService().getPlaybackIndecesList().size();
                } else {
                    mApp.getAudioPlaybackService().stopSelf();
                    return 0;
                }
            } catch( Exception e ) {
                e.printStackTrace();
    ...
    public void onResume() {
        ...
        //Update the seekbar.
        try {
            setSeekbarDuration( mApp.getAudioPlaybackService().getCurrentMediaPlayer().getDuration() / 1000 );
        } catch( Exception e ) {
            e.printStackTrace();
        }
    ...
    public void onStart() {
    	 ...
    	 */
        if( mApp.isServiceRunning() && mApp.getAudioPlaybackService().getCursor() != null ) {
            String[] updateFlags = new String[]{ Common.UPDATE_PAGER_POSTIION, Common.UPDATE_SEEKBAR_DURATION,
                                                 Common.HIDE_STREAMING_BAR, Common.INIT_PAGER,
                                                 Common.UPDATE_PLAYBACK_CONTROLS, Common.UPDATE_EQ_FRAGMENT };

            String[] flagValues = new String[]{ "" + mApp.getAudioPlaybackService().getCurrentSongIndex(),
