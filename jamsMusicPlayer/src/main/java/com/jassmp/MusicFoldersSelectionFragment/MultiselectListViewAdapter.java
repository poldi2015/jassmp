 ...
 */
package com.jassmp.MusicFoldersSelectionFragment;
...
public class MultiselectListViewAdapter extends ArrayAdapter<String> {
    ...
    public View getView( int position, View convertView, ViewGroup parent ) {
        ...
        if( mDirChecked ) {
            ...
            if( mFragment.getMusicFolders().get( folderPath ) != null
                && mFragment.getMusicFolders().get( folderPath ) == false ) {
            ...
            holder.fileFoldersCheckbox.setChecked( false );
            if( mFragment.getMusicFolders().get( folderPath ) != null
                && mFragment.getMusicFolders().get( folderPath ) == true ) {
                holder.fileFoldersCheckbox.setChecked( true );
            }
    ...
    private OnCheckedChangeListener checkChangeListener = new OnCheckedChangeListener() {
        ...
        public void onCheckedChanged( CompoundButton checkBox, boolean isChecked ) {
            ...
            if( checkBox.isPressed() ) {
                ...
                if( isChecked ) {
                    mFragment.getMusicFolders().put( filePath, true );
                } else if( mFragment.getMusicFolders().containsKey( filePath ) ) {
                    removeKeyAndSubFolders( filePath );
                } else {
                    mFragment.getMusicFolders().put( filePath, false );
                }
    ...
    private void removeKeyAndSubFolders( String key ) {
        ...
        Set<String> keySet = mFragment.getMusicFolders().keySet();
        ...
        for( int i = 0; i < keyArray.length; i++ ) {
            if( keyArray[ i ].startsWith( key ) ) {
                mFragment.getMusicFolders().remove( keyArray[ i ] );
            }
        }
