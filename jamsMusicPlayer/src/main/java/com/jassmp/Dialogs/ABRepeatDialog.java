...
import com.jassmp.Views.RangeSeekBar;
import com.jassmp.Views.RangeSeekBar.OnRangeSeekBarChangeListener;
...
public class ABRepeatDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog( Bundle savedInstanceState ) {
        ...
        currentSongIndex = mApp.getAudioPlaybackService().getCurrentSongIndex();
        ...
        currentSongDurationMillis = (int) mApp.getAudioPlaybackService().getCurrentMediaPlayer().getDuration();
        ...
        if( sharedPreferences.getInt( Common.REPEAT_MODE, Common.REPEAT_OFF ) == Common.A_B_REPEAT ) {
            repeatSongATime.setText(
                    convertMillisToMinsSecs( mApp.getAudioPlaybackService().getRepeatSongRangePointA() ) );
            repeatSongBTime.setText(
                    convertMillisToMinsSecs( mApp.getAudioPlaybackService().getRepeatSongRangePointB() ) );

            rangeSeekBar.setSelectedMinValue( mApp.getAudioPlaybackService().getRepeatSongRangePointA() );
            rangeSeekBar.setSelectedMaxValue( mApp.getAudioPlaybackService().getRepeatSongRangePointB() );

            repeatPointA = mApp.getAudioPlaybackService().getRepeatSongRangePointA();
            repeatPointB = mApp.getAudioPlaybackService().getRepeatSongRangePointB();
        } else {
            ...
            public void onClick( DialogInterface dialog, int which ) {
                if( ( currentSongDurationSecs - repeatPointB ) < mApp.getCrossfadeDuration() ) {
                    ...
                    mApp.getAudioPlaybackService()
                        .getHandler()
                        .removeCallbacks( mApp.getAudioPlaybackService().startCrossFadeRunnable );
                    mApp.getAudioPlaybackService()
                        .getHandler()
                        .removeCallbacks( mApp.getAudioPlaybackService().crossFadeRunnable );
                }

                mApp.broadcastUpdateUICommand( new String[]{ Common.UPDATE_PLAYBACK_CONTROLS }, new String[]{ "" } );
                mApp.getAudioPlaybackService().setRepeatSongRange( repeatPointA, repeatPointB );
                mApp.getAudioPlaybackService().setRepeatMode( Common.A_B_REPEAT );
